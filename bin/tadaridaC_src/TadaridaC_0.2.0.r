...
if (exists("ClassifEsp")==FALSE) ClassifEsp = load("ClassifEsp.learner")

CTP = read.table(file=fichier, header = TRUE, sep= "\t", dec = ".", fill = TRUE,comment.char = "")

#calcule probas par cris
ProbEsp0 <- predict(ClassifEsp, CTP,type="prob",norm.votes=TRUE)
...
#nettoie tous les objets sauf ClassifEsp
rm(list=setdiff(ls(), "ClassifEsp"))
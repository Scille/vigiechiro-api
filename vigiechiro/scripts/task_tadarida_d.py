...
TADARIDA_D_OPTS = settings.TADARIDA_D_OPTS
...
def tadaridaD(fichier_id):
    ...
    if fichier['mime'] not in ALLOWED_MIMES_WAV:
        logger.error('Fichier {} is not a wav file (mime: {})'.format(
    ...
    if r.status_code != 201:
        logger.error("{}'s .ta backend post {} error {} : {}".format(
            fichier_info, ta_payload, r.status_code, r.text))
        return 1
    ta_data = r.json()
    ta_fichier_info = '{} ({})'.format(ta_data['_id'], ta_data['titre'])
    logger.info("Registered {}'s .ta as {}".format(fichier_info, ta_fichier_info))
    ...
    if r.status_code != 200:
        logger.error('put to s3 {} error {} : {}'.format(s3_signed_url, r.status_code, r.text))
        return 1
